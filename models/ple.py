...
class PLEModel(torch.nn.Module):
    ...
    def __init__(self, categorical_field_dims, numerical_num, embed_dim, bottom_mlp_dims, tower_mlp_dims, task_num, shared_expert_num, specific_expert_num, dropout,gain=0.1):
        super().__init__()
        self.embedding = EmbeddingLayer(categorical_field_dims, embed_dim,gain=0.1)
        ...
        self.shared_module = nn.ModuleList([self.embedding, self.numerical_layer, self.share_experts, self.share_gates, self.task_experts])
        self.task_specific_module = nn.ModuleList([self.tower, self.task_gates])
    ...
    def get_embedding_output(self, categorical_x, numerical_x):
        categorical_emb = self.embedding(categorical_x)
        numerical_emb = self.numerical_layer(numerical_x).unsqueeze(1)
        emb = torch.cat([categorical_emb, numerical_emb], 1).view(-1, self.embed_output_dim)
        return emb