...
class SingleTaskModel(torch.nn.Module):
    ...
    def __init__(self, categorical_field_dims, numerical_num, embed_dim, bottom_mlp_dims, tower_mlp_dims, task_num, dropout,gain=0.1):
        super().__init__()
        self.embedding = torch.nn.ModuleList([EmbeddingLayer(categorical_field_dims, embed_dim, gain) for i in range(task_num)])
